###This will be faster by using linear interpolation built into griddata
...
from scipy.interpolate import RegularGridInterpolator as RGI  #horrible name...
...
def stepwise_selection(X, y, 
    ...
    while True:
        ...
        if worst_pval > threshold_out and excluded[worst_feature] in included and not 'Potassium' in excluded[worst_feature]:
            changed=True
            included.remove(excluded[worst_feature])
            dropped.append(excluded[worst_feature])
            if verbose:
...
#This function returns an interpolated matrix            
    ...
    print(string)
    inname = [f for f in os.listdir('.') if string in f and ((string + '_HAL') in f) and not (('_' + string) in f )  and '.csv' in f][0]
    infile = open(inname, 'r')
    inlines = infile.readlines()
    infile.close()
    splitlines = [line[:-1].split(',') for line in inlines[1:]]  #cut out the header
    splitlines = [[float(subentry) for subentry in entry[1:]] for entry in splitlines]  #cut out the first entry (index)
 ...
 #   char_len = int(len(splitlines)**(1/total_dims))      #lenght of each dimension...
    ...
    char_len = int(len(numpy_coords)**(1/total_dims))
    corner_delta = numpy_coords[-1] - numpy_coords[0]    #extremal corners
    grid_delta   = corner_delta/(char_len-1)             #distance between grids in each dimension
    ...
    for x in range(len(numpy_splitlines)):
        ...
        return_array[return_array_coord] = numpy_splitlines[x][-1]   #record as new numpy array
...
#    return_array = shrink(return_array, [downsampling_factor, downsampling_factor])
    ...
    griddata_object = np.array([entry for entry in splitlines])  #just a numpy array of positions and values for interpolation
    return return_array, griddata_object  #returns matrix version of HAL object as well as griddata version
...
def get_overall_derivative(beta, pd_name, varnames, interaction_mat_dict, X_percentile, column_names_signed, uncertainty_switch=False):
    ...
    #need to do all relevant points at once using griddata or is too slow
...
###Collect the partial derivative matrices and multiply by the global coefficients
    for key in interaction_mat_dict.keys():   #keys are sets of interactions
        if pd_name in key:      #then it will also be in the lower directories; this is individual feature in the interaction
            #print(key)
            deriv_mats = interaction_mat_dict[key][-1]   #dictionary pointing to the derivative matrices
            if pd_name in deriv_mats.keys():                 #this should always be true...
                ...
                gradient_interpolant = interaction_mat_dict[key][-1][pd_name][-1]
                responses = gradient_interpolant(submatrix)
...
#record the response of the ROSM over a percentile matrix
    ...
    for x in range(1, X_signed.shape[1]):   #evaluate for all but the constant
        ...
        interpolant = interaction_mat_dict[interaction_name][0]
        X_signed[:,x] = interpolant(submatrix)
...
sample_fraction2 = 1.0 #fraction of observations to keep for training

inname = sys.argv[1]
column_titles, splitlines, remove_cols = get_lines(inname, sample_fraction)
#remove_col_names = remove_col_names + [entry for entry in column_titles if '_' in entry and not('Phosphorus' in entry or 'Potassium' in entry)]   #just for testing the gradient
...
feature_column_titles, surface_column_titles = [entry for entry in column_titles if not '_' in entry], column_titles[:2] + [entry for entry in column_titles if '_' in entry] #and len(entry.split('_')) < 4]    #retain the latitude and longitude
